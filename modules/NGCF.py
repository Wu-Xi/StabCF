...
class NGCF(nn.Module):
    def __init__(self, data_config, args_config, adj_mat):
        ...
        *********************************************************
        ...
        self.user_embed = nn.Parameter(self.embedding_dict['user_emb'])
        self.item_embed = nn.Parameter(self.embedding_dict['item_emb'])
    ...
    def dns_negative_sampling(self, user_gcn_emb, item_gcn_emb, user, neg_candidates, pos_item):
        ...
        n_e = item_gcn_emb[neg_candidates]  # [batch_size, n_negs, n_hops+1, channel]
        if self.pool != 'concat':
            s_e = self.pooling(s_e)  # [batch_size, channel]
            n_e_ = self.pooling(n_e)  # [batch_size, n_negs, channel]
            scores = (s_e.unsqueeze(1) * n_e_).sum(-1)  # [batch_size, n_negs]
            indices = torch.max(scores, dim=1)[1].detach()  # [batch_size]
            return n_e[range(batch_size), indices, :]
        else:
            n_e_ = n_e  # [batch_size, n_negs, n_hops+1, channel]
            scores = (s_e.unsqueeze(1) * n_e_).sum(-1)  # [batch_size, n_negs, n_hops+1]
            indices = torch.max(scores, dim=1)[1].detach()  # [batch_size, n_hops+1]
            neg_items_emb_ = n_e.permute([0, 2, 1, 3])  # [batch_size, n_hops+1, n_negs, channel]
            neg_items_embedding_hardest = neg_items_emb_[[[i] for i in range(batch_size)],
                                                          range(neg_items_emb_.shape[1]), indices, :]  # [batch_size, n_hops+1, channel]
            return neg_items_embedding_hardest
