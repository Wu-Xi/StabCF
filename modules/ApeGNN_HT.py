...
class HeatKernel(nn.Module):
    ...
    def forward(self, batch=None, epoch=0):
        ...
        if self.ns == 'rns':  # n_negs = 1
            ...
            for k in range(self.K):
                neg_gcn_embs.append(self.dns_negative_sampling(user_gcn_emb, item_gcn_emb,
                                                                   user,
            ...
            neg_gcn_embs = []
            for k in range(self.K):
                neg_gcn_embs.append(self.mixgcf_negative_sampling(user_gcn_emb, item_gcn_emb,
            ...
            neg_gcn_embs = []
            for k in range(self.K):
                neg_gcn_embs.append(self.dins_negative_sampling(user_gcn_emb, item_gcn_emb,
                                                               user,
                                                               neg_item[:, k * self.n_negs: (k + 1) * self.n_negs],
                                                               pos_item))
            neg_gcn_embs = torch.stack(neg_gcn_embs, dim=1)
            ...
            neg_gcn_embs = []
            for k in range(self.K):
                neg_gcn_embs.append(self.dise_negative_sampling(epoch, user_gcn_emb, item_gcn_emb,
                                                                 user,
                                                                 neg_item[:, k * self.n_negs: (k + 1) * self.n_negs],
                                                                 pos_item))
            neg_gcn_embs = torch.stack(neg_gcn_embs, dim=1)
            ...
            pos_gcn_embs = []   
            for k in range(self.K):
                pos,neg = self.stabcf_negative_sampling(user_gcn_emb, item_gcn_emb,
                                                                      user,
                                                                      neg_item[:, k * self.n_negs: (k + 1) * self.n_negs],
                                                                      pos_item,user_pos)
                neg_gcn_embs.append(neg)
                pos_gcn_embs.append(pos)
            pos_gcn_embs = torch.stack(pos_gcn_embs, dim=1)
            neg_gcn_embs = torch.stack(neg_gcn_embs, dim=1)
        
        if self.ns == 'stabcf':
            pos_embs = pos_gcn_embs
        else:
            pos_embs = item_gcn_emb[pos_item]
        return self.create_bpr_loss(user_gcn_emb[user], pos_embs, neg_gcn_embs)
    ...
    def create_bpr_loss(self, user_gcn_emb, pos_gcn_embs, neg_gcn_embs):
        ...
        # 如果 pos_gcn_embs 有多余的 1 维，先 squeeze
        if pos_gcn_embs.dim() == 4 and pos_gcn_embs.shape[1] == 1:
            pos_gcn_embs = pos_gcn_embs.squeeze(1)
        ...
        if self.ns == 'dens' and self.gamma > 0.:
            pos_gcn_embs = pos_gcn_embs.float()
            neg_gcn_embs = neg_gcn_embs.float()
            user_gcn_emb = user_gcn_emb.float()
